...
- `subscription_status`: provider-neutral normalized states plus `none` and
  `restricted`.
...
- `unread_count`, read actor/time, `last_message_at`, assignment actor/time,
  intent evidence/classifier version, archive time, timestamps
...
- inbound provider event ID, sender/recipient addresses, and received time
...
- model/prompt version, grounding, risk flags, confidence, status, created_by,
  approval/rejection actors and times, edited body, transformation, parent
  suggestion, scheduled time, timestamps.

### Inbox participants, labels, and notes

- `conversation_participants` stores workspace-scoped contact/workspace identities,
  matched lead, normalized email/phone, display name, provider address, and audit
  timestamps.
- `inbox_labels` and `conversation_labels` provide workspace-owned tags with
  applying actor and timestamps.
- `conversation_notes` stores permissioned internal notes with author and timestamps.
- Every table has explicit grants, RLS, workspace indexes, and active-member read
  policies; writes require `inbox:reply`.

### Atomic Phase 5 workflows

- `private.process_inbound_message` deduplicates provider IDs, matches contacts,
  threads conversations, persists messages, classifies intent, updates unread/lead/
  sequence state, and appends message/audit events in one transaction.
- `private.stop_contact_from_conversation` atomically applies DNC, suppression,
  queued cancellation, sequence stop, conversation state, audit, and notification.
- `private.record_meeting_outcome` idempotently creates the calendar/meeting,
  updates lead/campaign/opportunity state and analytics, and appends an audit event.
- `private.claim_due_message` admits explicitly approved inbox replies while
  preserving subscription, suppression, consent, idempotency, and attempt gates.
...
- `id`, `workspace_id unique`, billing provider and provider customer,
  subscription, and product IDs
...
- `provider_event_id text primary key`, billing provider, provider mode, type,
  processing status
- optional workspace/subscription, payload hash and minimal redacted metadata,
  attempts, processed/error timestamps
- Never store unnecessary full provider payloads containing customer data.
...
- template, CTA, send hour, and follow-up step dimensions
- Unique dimensional key; generated by idempotent aggregation.
- `private.aggregate_daily_analytics` authoritatively rebuilds one workspace/day.
- `private.replenish_campaign_bounded` adds at most 100 eligible unsuppressed leads,
  once per campaign/day, while preserving approval requirements.
- Both functions are service-role-only. Optional template identifiers are validated
  as UUIDs before casting so malformed provider metadata cannot abort aggregation.
- Local execution remains Docker-blocked; static migration security tests pass.
...
Every migration must ultimately pass a clean `supabase db reset`, tenant-isolation
SQL tests, generated TypeScript types, and Supabase database/security advisors
before a remote push. During Phase 8, Docker is unavailable, so local runtime SQL,
RLS, and pgTAP execution remain blocked. Static migration security tests continue
to run and no hosted Supabase project is contacted.
...
## Phase 8 release-hardening migration

`20260724090000_phase8_release_hardening.sql` adds:

- `private.rate_limit_buckets` and atomic
  `private.consume_rate_limit(text, integer, integer)`;
- expiry, campaign-release-gate, and automated-grounding indexes;
- service-role-only `private.create_campaign` for one-transaction campaign,
  eligible-lead, channel, and audit creation;
- service-role-only `private.approve_message` and `private.revise_message` with
  row locks, optimistic timestamps, version history, and grounding review;
- service-role-only `private.transition_campaign` for atomic
  launch/pause/resume/kill transitions, complete schedules, approvals, grounding,
  suppression, WhatsApp consent, provider health, and audit logging.

All Phase 8 private functions fix `search_path`, avoid dynamic SQL, revoke execution
from `public`, `anon`, and `authenticated`, and grant only to `service_role`.
Production callers still authenticate the user, resolve the active workspace, and
enforce permissions before invoking an RPC. The Data API exposes the private schema
only so trusted service-role RPC calls can reach these functions; table/function
grants remain revoked from client roles.
