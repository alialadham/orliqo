...
test("protected redirect, demo controls, responsive navigation, and logout", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("heading", {
      name: "Find the right businesses. Reach them personally.",
    }),
  ).toBeVisible();
  ...
  if (testInfo.project.name !== "desktop-chromium") {
    ...
    expect(
      Math.abs((dialogBox?.width ?? 0) - (viewport?.width ?? 0)),
...
test("registration validates and creates a demo onboarding session", async ({
  page,
}) => {
  const failures = collectClientFailures(page);
  ...
  await expect(
    page.getByText("You must agree to the Terms and Privacy Policy."),
  ).toBeVisible();
  ...
  await page
    .getByRole("checkbox", { name: "I agree to the Terms and Privacy Policy." })
    .click();
  await page.getByRole("button", { name: "Create My Workspace" }).click();
  ...
  await expect(page.getByTestId("company-name")).toHaveValue(
    "Orliqo Test Workspace",
  );
  expect(failures).toEqual([]);
});

test("workspace switching enforces viewer billing restrictions", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Desktop workspace switcher workflow",
  );
  const failures = collectClientFailures(page);
  ...
  await page
    .getByRole("button", { name: "Northstar Demo Viewer starter" })
    .click();
  await expect(
    page.getByRole("button", { name: "Northstar Demo", exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Your viewer role cannot access billing in this workspace."),
  ).toBeVisible();

  await page
    .getByRole("button", { name: "Northstar Demo", exact: true })
    .click();
  await page.getByRole("button", { name: "Orliqo Demo Owner growth" }).click();
  await expect(
    page.getByRole("button", { name: "Orliqo Demo", exact: true }),
  ).toBeVisible();
  await page.goto("/app/billing");
  await expect(
    page.getByRole("heading", { name: "Billing and plans", exact: true }),
  ).toBeVisible();
  expect(failures).toEqual([]);
});
