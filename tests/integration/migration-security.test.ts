...
describe("migration security baseline", () => {
  it("includes every public table in the RLS enablement migration", () => {
    const createdTables = [...migrationSql.matchAll(/create table public\.([a-z_]+)/g)].map((match) => match[1]);
    const rlsStatementIndex = migrationSql.indexOf("alter table public.%I enable row level security");
    const rlsBlockStart = migrationSql.lastIndexOf("foreach table_name in array array[", rlsStatementIndex);
    ...
    const missing = createdTables.filter((table) => !rlsBlock.includes(`'${table}'`));
    expect(createdTables.length).toBeGreaterThan(50);
    expect(missing).toEqual([]);
  });

  it("keeps credential and provider internals out of client grants", () => {
    expect(migrationSql).toContain("revoke all on private.integration_credentials from public, anon, authenticated");
    expect(migrationSql).not.toMatch(/grant .*private\.integration_credentials to authenticated/i);
    expect(migrationSql).not.toMatch(/grant .*public\.billing_events.* to authenticated/i);
    expect(migrationSql).not.toMatch(/grant .*public\.provider_webhook_events.* to authenticated/i);
  });
