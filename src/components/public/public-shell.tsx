...
export function PublicDocument({ eyebrow, title, summary, children }: { eyebrow: string; title: string; summary: string; children: React.ReactNode }) {
  return (
    <div className="min-h-dvh bg-background"><PublicHeader /><main id="main-content" tabIndex={-1} className="mx-auto max-w-4xl px-5 py-16 outline-none sm:px-7 sm:py-24"><p className="text-sm font-semibold tracking-[0.12em] text-primary uppercase">{eyebrow}</p><h1 className="mt-4 text-4xl font-bold sm:text-5xl">{title}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">{summary}</p><div className="mt-12 space-y-8 border-t pt-10 text-[15px] leading-7 text-foreground/80 [&_a]:font-medium [&_a]:text-primary [&_a]:underline-offset-4 [&_a:hover]:underline [&_h2]:mb-2 [&_h2]:text-xl [&_h2]:font-bold [&_li]:text-muted-foreground [&_p]:text-muted-foreground [&_ul]:list-disc [&_ul]:space-y-1 [&_ul]:pl-6">{children}</div></main><PublicFooter /></div>
  );
}
