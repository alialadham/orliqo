...
import type { WorkspaceContext, WorkspaceSummary } from "@/features/workspaces/types";
...
export const getWorkspaceContext = cache(async (): Promise<WorkspaceContext | null> => {
  ...
  if (environment.demoMode && user.provider === "demo") {
    const session = await readDemoSession();
    if (!session || session.kind !== "workspace") return null;

    const activeWorkspace =
      DEMO_WORKSPACES.find((workspace) => workspace.id === session.activeWorkspaceId) ??
      DEMO_WORKSPACES[0];
  ...
  const fullName = profileResult.data?.full_name || user.fullName || user.email.split("@")[0] || "Orliqo user";
