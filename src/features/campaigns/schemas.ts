...
}).superRefine((value, ctx) => { if (value.maxIntervalMinutes < value.minIntervalMinutes) ctx.addIssue({ code: "custom", path: ["maxIntervalMinutes"], message: "Maximum interval must be at least the minimum." }); if (value.sendWindowEnd <= value.sendWindowStart) ctx.addIssue({ code: "custom", path: ["sendWindowEnd"], message: "Send window must end after it starts." }); });
