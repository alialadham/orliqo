...
export default async function DiscoveryPage({ searchParams }: { searchParams: Promise<{ q?: string; industry?: string; city?: string; minScore?: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-5"><div><div className="flex items-center gap-2"><h1 className="text-3xl font-bold">Discovery</h1><Badge variant="outline">Evidence-backed</Badge></div><p className="mt-1 text-sm text-muted-foreground">Explore qualified records using transparent scores and stored sources.</p></div>
