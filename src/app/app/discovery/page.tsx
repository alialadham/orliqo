...
export default async function DiscoveryPage({ searchParams }: { searchParams: Promise<{ q?: string; industry?: string; city?: string; minScore?: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-5"><div><div className="flex items-center gap-2"><h1 className="text-3xl font-bold">Discovery</h1><Badge variant="outline">Evidence-backed</Badge></div><p className="mt-1 text-sm text-muted-foreground">Explore qualified records using deterministic scores and stored sources. Demo mode never contacts external businesses.</p></div>
    <form className="grid gap-2 rounded-xl border bg-card p-4 sm:grid-cols-[1.5fr_1fr_1fr_130px_auto]"><label className="relative"><Search className="absolute top-2 left-2.5 size-4 text-muted-foreground" /><Input name="q" defaultValue={params.q} className="pl-8" placeholder="Search opportunity" /></label><Input name="industry" defaultValue={params.industry} placeholder="Industry" /><Input name="city" defaultValue={params.city} placeholder="City" /><Input name="minScore" type="number" min="0" max="100" defaultValue={params.minScore ?? "60"} placeholder="Min score" /><Button type="submit"><SlidersHorizontal />Research</Button></form>
