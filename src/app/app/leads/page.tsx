...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <div className="mx-auto w-full min-w-0 max-w-[1600px] overflow-x-hidden space-y-5">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><h1 className="text-3xl font-bold">Leads</h1><p className="mt-1 text-sm text-muted-foreground">Qualify, verify, assign, and protect every prospect before outreach.</p></div><Button asChild variant="outline"><Link href="/app/leads/import"><FileSpreadsheet />Import CSV/XLSX</Link></Button></div>
    ...
    <form aria-label="Lead filters" method="get" className="rounded-xl border bg-card p-3"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(220px,1.5fr)_repeat(5,minmax(120px,1fr))_auto]"><label className="relative sm:col-span-2 xl:col-span-1"><span className="sr-only">Search business, email, or city</span><Search className="absolute top-2 left-2.5 size-4 text-muted-foreground" aria-hidden="true" /><Input name="q" defaultValue={query.q} className="pl-8" placeholder="Search business, email, or city" /></label><Input aria-label="Industry" name="industry" defaultValue={query.industry} placeholder="Industry" /><Input aria-label="Country" name="country" defaultValue={query.country} placeholder="Country" /><Input aria-label="City" name="city" defaultValue={query.city} placeholder="City" /><select aria-label="Lead status" name="status" defaultValue={query.status} className="h-8 rounded-lg border bg-transparent px-2 text-sm"><option value="">All statuses</option>{["new", "qualified", "contacted", "replied", "interested", "do_not_contact", "archived"].map((item) => <option key={item}>{item}</option>)}</select><Input aria-label="Minimum qualification score" name="minScore" type="number" min="0" max="100" defaultValue={query.minScore} placeholder="Min score" /><Button type="submit"><Filter aria-hidden="true" />Filter</Button></div><details className="mt-3"><summary className="cursor-pointer text-xs font-medium text-muted-foreground">More filters</summary><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5"><select aria-label="Website status" name="websiteStatus" defaultValue={query.websiteStatus} className="h-8 rounded-lg border bg-transparent px-2 text-sm"><option value="">Any website status</option>{["healthy", "unknown", "no_website", "outdated", "poor_mobile", "slow", "no_booking"].map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Contact availability" name="contact" defaultValue={query.contact} className="h-8 rounded-lg border bg-transparent px-2 text-sm"><option value="">Any contact</option><option value="email">Has email</option><option value="phone">Has phone</option><option value="instagram">Has Instagram</option></select><select aria-label="Suppression status" name="doNotContact" defaultValue={query.doNotContact === undefined ? "" : String(query.doNotContact)} className="h-8 rounded-lg border bg-transparent px-2 text-sm"><option value="">Any suppression</option><option value="false">Contactable</option><option value="true">Do not contact</option></select><select aria-label="Sort leads" name="sort" defaultValue={query.sort} className="h-8 rounded-lg border bg-transparent px-2 text-sm"><option value="score">Sort by score</option><option value="business">Sort by business</option><option value="activity">Sort by activity</option></select><Button asChild type="button" variant="ghost"><Link href="/app/leads">Clear filters</Link></Button></div></details></form>
